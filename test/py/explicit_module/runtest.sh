...
PYTHONPATH=./gen-py python3 -c 'import foo.bar.baz' || exit 1
PYTHONPATH=./gen-py python3 -c 'import test2' || exit 1
PYTHONPATH=./gen-py python3 -c 'import test1' &>/dev/null && exit 1  # Should fail.
cp -r gen-py simple
